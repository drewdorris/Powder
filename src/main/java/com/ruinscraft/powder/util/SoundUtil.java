	...
	public static List<SoundEffect> getSoundEffectsFromNBS(String fileName, double volume,
		...
		if (fileName.contains("/")) {
			...
			try {
				...
				PowderPlugin.get().getLogger().info("AAAAA");
				e.printStackTrace();
				return null;
			}
			...
			File file = new File(PowderPlugin.get().getDataFolder() + "/songs", fileName);
			if (!file.exists()) {
				PowderPlugin.get().saveResource("songs/" + fileName, false);
			}
